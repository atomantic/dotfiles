...
bot "OK, now I am going to set up your git identity in ~/.gitconfig.local:"
read -r -p "Do you want to configure your git identity? [y|N] " update_gitconfig_response
if [[ $update_gitconfig_response =~ (yes|y|Y) ]]; then
  if [[ -f "$gitconfig_local" ]] && grep -q '^\[user\]' "$gitconfig_local"; then
    ok "~/.gitconfig.local already defines your git identity; leaving it untouched."
  else
    ...
    running "writing your git identity to $gitconfig_local ($COL_YELLOW$fullname, $email, $githubuser$COL_RESET)"

    cat > "$gitconfig_local" <<EOF
# Local git identity — NOT tracked by the dotfiles repo.
# Generated by install.sh; edit freely. Pulled in via [include] in ~/.gitconfig.
[user]
	name = $fullname
	email = $email
[github]
...
# sudo pmset -a standbydelaylow 86400;ok
...
# sysadminctl -screenLock immediate -password -;ok
...
# #running "Add the keyboard shortcut ⌘ + Enter to send an email in Mail.app"#defaults write com.apple.mail NSUserKeyEquivalents -dict-add "Send" -string "@\U21a9";ok
...
for app in "Activity Monitor" "Address Book" "Calendar" "Contacts" "cfprefsd" \
  "Dock" "Finder" "Messages" "Safari" "SizeUp" "SystemUIServer" "Terminal"; do
